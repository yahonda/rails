*   `check_all_foreign_keys_valid!` no longer requires superuser privileges on
    ...
    On PostgreSQL 18.4+, foreign keys in the current schemas are revalidated by
    toggling each constraint `NOT ENFORCED` and back to `ENFORCED`, which forces a
    full re-check without updating the system catalog directly. Earlier PostgreSQL
    versions keep using `UPDATE pg_catalog.pg_constraint` followed by
    `VALIDATE CONSTRAINT`.
