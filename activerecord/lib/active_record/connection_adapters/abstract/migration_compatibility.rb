...
module ActiveRecord
  class Migration
    module Compatibility # :nodoc: all
      ...
      # define +V<major>_<minor>+ submodules that override migration methods to
      # replicate historical behavior. The submodule names must mirror the
      # versioned classes under +ActiveRecord::Migration::Compatibility+
      # (+V5_0+, +V5_1+, +V6_0+, ...). No registration or lookup table is
      # needed; +module_for+ discovers the submodules by enumerating constants.
      #
      # For a given migration class, +module_for+ returns a new anonymous
      # module that includes every submodule whose paired
      # +Compatibility::V<major>_<minor>+ class is an ancestor of the migration
      # class. Submodules are included in class-hierarchy order so the
      # oldest-era submodule ends up at the top of the ancestors chain,
      # mirroring the fact that older-version classes are the most derived in
