          ...
          if supports_enforced_foreign_keys?
            ...
            begin
              ...
              enforced_fks.each do |fk|
                ...
                # InvalidForeignKey is a subclass of StatementInvalid, so rescue it first
                # and re-raise to surface a meaningful FK violation error.
                raise
              rescue ActiveRecord::StatementInvalid
                # Typically raised when the caller's transaction is in an aborted state;
                # the subsequent rollback will undo the `NOT ENFORCED` changes automatically.
                break
              end
            end
          else
          ...
          if supports_enforced_foreign_keys?
            enforced_join = <<~SQL
              JOIN pg_catalog.pg_constraint c
               ...
               AND c.connamespace::regnamespace::text = tc.constraint_schema
               AND c.conenforced = true
